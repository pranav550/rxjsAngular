...
const routes: Routes = [
  ...
  { path: "createObservable", component: CreateObservableComponent },
  {
    ...
    loadChildren: () =>
      import("./featured/create-observable/create-observable.module").then(
        (m) => m.CreateObservableModule
      ),
  },
  {
    path: "opertors",
    loadChildren: () =>
      import("./featured/operators/operators.module").then(
        (m) => m.OperatorsModule
      ),
  },
];
...
@NgModule({
  imports: [RouterModule.forRoot(routes), CreateObservableModule],
